...
2026-05-04 (M4 code complete — owner setup pending)
...
| 4 | Priorities table + Council Home (Priorities List) read-only | 🔨 | **Code complete; awaiting owner migration + verification.** New: `priorities` table in `src/db/schema.ts` (`Priority` + `PriorityIcon` types exported) + migration `drizzle/migrations/0002_priorities.sql`. Read-only data access in `src/lib/priorities.ts` (`getPrioritiesForUser` filters `deleted_at IS NULL`, orders by `position`). Council Home at `src/app/priorities/page.tsx` with `PriorityCard.tsx` component (color dot + name + status badge + min–max minutes/week). Empty state shown when 0 Priorities — copy is honest about the M5 Create button. Root `/` now redirects authed users to `/priorities`, unauthed to `/signin`. Out-of-scope (deferred): drag handles + Create button + edit/pause/archive UI (M5), quarter info in header (M7), planning banners (M11+), last-activity tracking (M8+). Typecheck passes. **Owner still needs**: (a) apply `drizzle/migrations/0002_priorities.sql` in Neon's SQL editor, (b) merge feature branch to main once M4 verified. |
...
- **Owner setup needed to close M4**:
  1. **Apply migration**: in Neon's SQL editor, paste and run the contents of `drizzle/migrations/0002_priorities.sql`. Creates the `priorities` table + two indexes.
  2. After verification on the production URL (empty state visible after deploy + optional: paste seed SQL to render fake cards), merge `claude/read-prompt-mobile-xyEk8` to `main`.
