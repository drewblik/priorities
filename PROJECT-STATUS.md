...
2026-05-04 (M6 code complete — owner setup pending)
...
| 6 | Priority Detail page with full edit | 🔨 | **Code complete; awaiting owner migration + Vercel Blob provisioning + verification.** New tables: `priority_memory` + `priority_files` (`src/db/schema.ts` + `drizzle/migrations/0003_priority_memory_and_files.sql`). New libs: `src/lib/priority-memory.ts`, `src/lib/priority-files.ts`. New API routes: POST /api/priorities/[id]/memory, POST/PATCH/DELETE /api/priorities/[id]/memory/[mid], POST /api/priorities/[id]/files (multipart, `@vercel/blob` integration), POST/DELETE /api/priorities/[id]/files/[fid]. New UI: `/priorities/[id]/page.tsx` (Detail page) consolidates the M5 edit form + collapsible Memory + Files sections + Danger zone; old `/priorities/[id]/edit` now redirects to the canonical detail URL. Components: `MemorySection.tsx` (server, with add form), `MemoryEntry.tsx` (client, inline edit ↔ display toggle, react-markdown + rehype-sanitize rendering), `FilesSection.tsx` (server, with multipart upload form + list + delete). PriorityForm extended with `pinnedSummary` textarea (visible only on edit). softDeletePriority now cascades via `db.transaction()` to memory + files. SortablePriorityCard kebab "Edit" → "Open" link points to `/priorities/[id]`. File constraints: 10MB max, 9-MIME whitelist (image/jpeg/png/gif/webp + pdf + plain/markdown/csv + json), public Blob URLs. Graceful "Vercel Blob not configured" placeholder when `BLOB_READ_WRITE_TOKEN` is unset. Typecheck passes. **Owner still needs**: (a) apply `drizzle/migrations/0003_priority_memory_and_files.sql` in Neon, (b) provision Vercel Blob in dashboard (Storage → Create Database → Blob), (c) trigger redeploy. |
...
- **Owner setup needed to close M6**:
  1. **Apply migration**: paste and run `drizzle/migrations/0003_priority_memory_and_files.sql` in Neon's SQL editor. Creates `priority_memory` + `priority_files` tables with their partial indexes.
  2. **Provision Vercel Blob**: Vercel dashboard → priorities project → Storage → Create Database → Blob → name it (e.g. `priorities-files`) → Create. Vercel auto-creates the `BLOB_READ_WRITE_TOKEN` env var; verify it shows up in Settings → Environment Variables for All Environments.
  3. **Redeploy** so the new env var reaches running serverless functions.
  4. After all three, verify on `priorities-two.vercel.app`: open a Priority, add memory, edit memory, delete memory, upload file, delete file. Confirm the cascade by deleting the whole Priority and checking Neon SQL.
