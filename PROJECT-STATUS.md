...
2026-05-05 (M7 code complete — owner verification pending)
...
| 7 | Quarters table + first-quarter calculation logic + display in Priorities List header | 🔨 | **Code complete; awaiting Neon migration + verification.** New: `quarters` Drizzle table + `drizzle/migrations/0004_quarters.sql` (text id, user_id FK, quarter_label, start_date/end_date as `date`, status active\|closed, is_partial, soft-delete, partial unique index on `(user_id) WHERE status='active' AND deleted_at IS NULL`). New `src/lib/quarters.ts` with pure helpers (`currentDateInTz` via `date-fns-tz`, `calendarQuarterBounds`, `weeksInQuarter`, `weekNumber`) and DB-backed `getActiveQuarter` + `ensureCurrentQuarter` (lazy rollover: close + insert sequentially, no transaction since Neon HTTP can't branch on intermediate results). Council Home (`src/app/priorities/page.tsx`) calls `ensureCurrentQuarter` in parallel with `getPrioritiesForUser` and renders a small `Q2 2026 · week N of K (partial)` line under the title. Manual "start new quarter early" + "Plan your new quarter" banner deferred (M11/M14). Typecheck passes. **Owner still needs**: apply `drizzle/migrations/0004_quarters.sql` in Neon. |
...
- **Owner setup needed to close M7**: apply `drizzle/migrations/0004_quarters.sql` in Neon's SQL editor. Then merge feature branch and verify on `priorities-two.vercel.app`.
