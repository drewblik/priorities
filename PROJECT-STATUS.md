...
2026-05-04 (M5 code complete — owner verification pending)
...
| 5 | Manual Priority CRUD + drag-to-reorder (@dnd-kit) + pause/archive | 🔨 | **Code complete; awaiting owner verification.** No schema change. New: data-access (`createPriority`, `updatePriority`, `softDeletePriority`, `reorderPriorities` in `src/lib/priorities.ts`); zod schemas + form-payload helper (`src/lib/priorities-validation.ts`); API routes (`POST /api/priorities`, `PATCH+DELETE /api/priorities/[id]`, `POST /api/priorities/reorder`) accepting JSON or form-urlencoded; Create + Edit full-page forms (`src/app/priorities/new/page.tsx`, `.../[id]/edit/page.tsx`); shared form components (`PriorityForm.tsx`, `IconPicker.tsx` with 8 preset colors × 4 letter styles + live "P" preview); sortable list (`PrioritiesList.tsx` + `SortablePriorityCard.tsx`) using `@dnd-kit/core` + `@dnd-kit/sortable` with optimistic UI + revert-on-failure toast (TouchSensor delay 250ms for mobile long-press); per-card kebab menu (Edit / Pause↔Resume / Archive↔Reactivate / Delete with confirm); `/priorities` Council Home now reads `?archived=1` toggle, shows Create Priority CTA, surfaces success/error toasts via redirect query params; `PriorityCard` upgraded from a color dot to a styled "P" letter icon. Soft-delete only sets `deleted_at` on the Priority — cascade hook commented for M8 (Tasks/Events). Validation: name 1–120, strategies 0–2000 each, minutes 0–10000 with `min ≤ max`, cadence non-empty subset, color in preset palette, style in 4-style enum. Concurrency: last-write-wins (single-user v1). Typecheck passes. |
...
- **Owner verification needed to close M5**: no migration, no env var. Just merge `claude/read-prompt-mobile-xyEk8` to `main` once typecheck-clean (already done locally), let Vercel auto-deploy, and walk through the verification checklist (Create / Edit / Reorder / Pause / Archive / Show archived / Delete / Validation) on `priorities-two.vercel.app`.
