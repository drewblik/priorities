...
export type DailyChatPanelInitial = {
...
export function ChatPanel({ initial }: Props) {
  ...
  if (!initial.currentPriority || !initial.sessionId) {
    return (
      <div className="rounded-md border border-green-600/30 bg-green-600/5 p-4 text-sm text-green-700">
        All Priorities planned for tomorrow. Tap{' '}
  ...
  const tasksLine =
    initial.taskCount === 0
      ...
      : initial.taskCount === 1
        ...
        : `You have ${initial.taskCount} tasks to time-block for tomorrow.`;
  const opener =
    messages.length === 0
      ? `Let's plan tomorrow (${initial.dateLabel}) for **${priority.name}**.\n\n${tasksLine}\n\nSuggest start/end times that fit around what's already blocked. We'll lock them in one at a time.`
      : null;
  ...
  async function send(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/plan/day/chat', {
        method: 'POST',
  ...
  function applySseEvent(event: SseEvent) {
    ...
    if (event.type === 'lock_busy') {
      setBanner({
        tone: 'warning',
        message: `Another daily-planning session is in progress. Try again in ~${Math.ceil(event.try_again_in_ms / 1000)}s.`,
      });
      return;
    }
    if (event.type === 'error') {
  ...
  async function finishOrSkip(action: 'finish' | 'skip') {
    ...
    setStreaming(true);
    try {
      const res = await fetch(`/api/plan/day/${action}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        ...
        {signaledDone ? (
          <div className="space-y-2 rounded-md border border-green-600/30 bg-green-600/5 p-3">
            <p className="text-sm text-green-700">
              {priority.name} has signaled it&apos;s done planning tomorrow.
          ...
          <form onSubmit={send} className="space-y-2">
            <textarea
              ...
              maxLength={4000}
              disabled={streaming}
              placeholder={messages.length === 0 ? 'Reply with proposed time blocks…' : 'Reply…'}
            ...
            <p className="text-xs text-muted-foreground">
              ...
              you&apos;re ready to move on. Time blocks already saved stay;
              this just closes the conversation for this Priority.
            </p>
          </form>
        )}
      </div>
