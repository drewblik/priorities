...
export type ChatPanelInitial = {
  sessionId: string | null;
  currentPriority: { id: string; name: string; color: string } | null;
  totalWeeks: number;
  /** Pre-rendered text of any prior turns from chat_messages. v1 keeps it
   *  simple: each entry is { role, text }. Tool calls in past turns are
   *  not surfaced here — they only render as live pills during the
   *  active stream. */
...
export function ChatPanel({ initial, quarterId }: Props) {
  ...
  if (!initial.currentPriority || !initial.sessionId) {
    return (
      <div className="rounded-md border border-green-600/30 bg-green-600/5 p-4 text-sm text-green-700">
        All priorities planned for this quarter. Tap{' '}
  ...
  async function send(e: React.FormEvent) {
    ...
    // Optimistic append of user + empty assistant placeholder.
    setMessages((prev) => [
      ...prev,
    ...
    try {
      const res = await fetch('/api/plan/quarter/chat', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        setBanner({
          tone: 'error',
          message: j?.message ?? `Chat failed (${res.status}).`,
        });
  ...
  function applySseEvent(event: SseEvent) {
    ...
    if (event.type === 'tool_result') {
      ...
      // After a successful tool result, refresh the page's server-rendered
      // QuarterCalendar so the new chip appears. router.refresh() re-runs
      // the server component without a full nav.
      if (event.ok) router.refresh();
      return;
    }
    if (event.type === 'message_done') {
      // No-op; we already streamed text + tool events.
      return;
    }
    ...
    if (event.type === 'lock_busy') {
      setBanner({
        tone: 'warning',
        message: `Another quarter-planning session is in progress. Try again in ~${Math.ceil(event.try_again_in_ms / 1000)}s.`,
      });
      return;
    }
    if (event.type === 'error') {
  ...
  async function finishOrSkip(action: 'finish' | 'skip') {
    ...
    setStreaming(true);
    try {
      const res = await fetch(`/api/plan/quarter/${action}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      // Re-render server component with new session/priority.
      router.refresh();
    } catch (err) {
      setBanner({
        tone: 'error',
        ...
        <ul className="space-y-3">
          ...
          {messages.map((m, i) => (
            ...
            >
              ...
              <div className="mt-1 whitespace-pre-wrap">{m.text || (streaming && i === messages.length - 1 ? '…' : '')}</div>
              {m.tools.length > 0 ? (
