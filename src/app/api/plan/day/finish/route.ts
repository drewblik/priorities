...
import { getDailyPlanningQueue } from '@/lib/priorities';
...
export async function POST(req: Request) {
  ...
  if (!chat || chat.sessionType !== 'daily' || !chat.contextRef) {
    return NextResponse.json({ error: 'session_not_found' }, { status: 404 });
  }

  await closeSession(session.user.id, sessionId);

  const queue = await getDailyPlanningQueue(session.user.id);
  const closed = await getClosedSessions(session.user.id, 'daily', chat.contextRef);
