...
import { loadDailyContext } from '@/lib/daily-context';
import { isIsoDate } from '@/lib/daily-utils';
import { buildDailySystemPrompt } from '@/lib/daily-planning-prompt';
import {
  DAILY_PLANNING_TOOLS,
  executeDailyTool,
} from '@/lib/daily-planning-tools';
...
export async function POST(req: Request) {
  ...
  if (!chatSession || chatSession.sessionType !== 'daily' || chatSession.closedAt !== null) {
    return jsonError(404, 'session_not_found', 'No open daily session.');
  }
  if (!chatSession.priorityId || !chatSession.contextRef) {
    return jsonError(400, 'session_misconfigured', 'Session missing priority or day ref.');
  }
  if (!isIsoDate(chatSession.contextRef)) {
    return jsonError(400, 'session_misconfigured', 'Session day ref is not a valid date.');
  }

  const dateISO = chatSession.contextRef;
  ...
  const lockKey = `plan:daily:${dateISO}`;
      ...
      try {
        for (let turn = 0; turn < MAX_TOOL_LOOP_TURNS; turn++) {
          ...
          for (const block of toolUseBlocks) {
            ...
            const result = await executeDailyTool(block.name, block.input, toolCtx);
            send(
              result.ok
        ...
        console.error('daily-plan stream error:', message);
