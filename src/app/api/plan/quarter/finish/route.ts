...
/**
 * Close the current quarter-planning session for the active priority and
 * advance to the next un-completed priority in the queue. Returns the new
 * sessionId + currentPriorityId, or null if the queue is done.
 */
export async function POST(req: Request) {
  ...
  if (!chat || chat.sessionType !== 'quarter' || !chat.contextRef) {
    return NextResponse.json({ error: 'session_not_found' }, { status: 404 });
  }

  await closeSession(session.user.id, sessionId);

  // Advance: find the first queued priority that doesn't have a closed session.
  const queue = await getQuarterlyPlanningQueue(session.user.id);
  const closed = await getClosedSessions(session.user.id, 'quarter', chat.contextRef);
